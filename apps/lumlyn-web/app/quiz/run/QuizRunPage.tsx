...
/**
 * Props folosite în modul overlay pentru a intercepta navigarea. Atunci când
 * `onComplete` este furnizat, componenta va apela callback‑ul cu varianta
 * calculată în loc să navigheze la `/quiz/outro`. De asemenea, `onBack`
 * permite interceptarea acțiunii de back pe primul ecran.
 */
...
export default function QuizRunPage({ onComplete, onBack }: QuizRunPageProps = {}): JSX.Element {
  ...
  const handleBack = React.useCallback(() => {
    if (state.step === "question" && state.index === 0) {
      ...
      if (onBack) {
        // dacă vine din overlay, închidem overlay-ul (revine la teaser)
        onBack();
      } else {
        router.back();
      }
    ...
    dispatch({ type: "BACK" });
  }, [state.step, state.index, router, onBack]);
